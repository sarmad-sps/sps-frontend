...
const ServicesSection: React.FC = () => {
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    if (diff > 50) { setPage((prev) => (prev + 1) % totalPages); setStartX(null); }
    else if (diff < -50) { setPage((prev) => (prev - 1 + totalPages) % totalPages); setStartX(null); }
  };
  const handleMouseDown = (e: React.MouseEvent) => { setIsDragging(true); setStartX(e.clientX); };
  const handleMouseMove = (e: React.MouseEvent) => {
    if (!isDragging || startX === null) return;
    const diff = startX - e.clientX;
    if (diff > 50) { setPage((prev) => (prev + 1) % totalPages); setIsDragging(false); setStartX(null); }
    else if (diff < -50) { setPage((prev) => (prev - 1 + totalPages) % totalPages); setIsDragging(false); setStartX(null); }
  };
  const handleMouseUp = () => { setIsDragging(false); setStartX(null); };
              ...
              {slideServices.map((service: Service) => (
                <div
                  ...
                  className={`flex-shrink-0 p-3 ${
                    cardsPerPage === 1 ? "w-full" : cardsPerPage === 2 ? "w-1/2" : "w-1/4"
                  }`}
                >
                  <div className="group bg-white rounded-lg shadow-md p-6 flex flex-col items-center text-center transition-all duration-500 hover:bg-[#1894A4] hover:text-white h-full">
                    <img
                      src={service.iconSrc}
                      alt={service.title}
                      className={`${
                        service.id === "01" || service.id === "03" || service.id === "04"
                          ? "w-18 h-16"
                          : "w-16 h-16"
                      } mb-4 transition-all duration-500 ${service.id === "02" ? "filter brightness-0" : ""} group-hover:filter group-hover:brightness-0 group-hover:invert`}
                    />
                    ...
                    <Link
                      to={service.route}
                      className="border border-[#1894A4] text-[#1894A4] px-4 py-2 rounded transition-colors duration-500 group-hover:bg-white group-hover:text-[#1894A4] mt-auto"
                    >
                      Read more
                    </Link>
                  </div>
